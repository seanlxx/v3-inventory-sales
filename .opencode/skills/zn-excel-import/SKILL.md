...
> 1 号机 / 2 号机的销售来源全靠 zn 平台后台导出的 Excel。本 skill 把两份 Excel 的字段映射 + 幂等键 + 批次约束写成可执行规则，避免每次都重新摸索。
>
...
| Phase 2 后写真实 `machine_id`（'1号机' / '2号机'） | 仍写折叠值 `'1/2号机'` |
...
**Phase 2 关键变更**：导入时写入 sales_orders.machine_id 必须用 `mapZnDeviceToMachine(deviceCode)` 的真实返回值（'1号机' / '2号机'），**不要再走 `stockMachineIdFor`**（该文件 Phase 2 删除）。
