...
| 4   | 1.5d | 总库存展示 + 看板默认汇总 | 商品/库存/仪表盘适配 |
| 5   | 1d   | `scripts/deploy-pages.ps1` | 上线 + 部署前重跑 5 项对账 |
...
| D5 | 前端库存默认视图 | **A 库存列表只显示商品总库存**；按机余额仅用于调拨、盘点、销售归属 |
...
| 修改 | 内容 |
| --- | --- |
| 🔧 `frontend/app/pages/products.vue` | 商品表格只显示总库存，不展示 1/2/3 号机库存列 |
| 🔧 `frontend/app/pages/inventory.vue` | 库存列表按商品汇总总库存；调拨/盘点仍读取按机余额 |
| 🔧 `frontend/app/pages/dashboard.vue` | 全机汇总 / 按机切分切换 |
| 🔧 `frontend/app/types/product.ts` | 后端可保留 `inventoryByMachine` 兼容字段；前端不展示按机库存列 |
...
| R3 | balance 是缓存 | Phase 2（合并到 `_shared/inventory-balance.js`） |
| R4 | 负库存恢复分支 | Phase 2（统一一种策略） |
| R5 | 作废未反冲 | Phase 0.5 发现 + Phase 2 重建 |
| R6 | adjustment 无凭据 | Phase 0.5 发现 + Phase 3 盘点流程 |
| R7 | 同名重复商品 | Phase 0.5 发现 + L3 partial unique index |
