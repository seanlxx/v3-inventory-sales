...
assert.equal(
  ...
  Math.min(500, Math.max(0, expectedStats.orders - 500))
);
...
assert.equal(m1, expectedStats.byMachine['1号机'], '1号机有效订单应完整导入');
assert.equal(m2, expectedStats.byMachine['2号机'], '2号机有效订单应完整导入');
...
assert.equal(chunkedM1, expectedStats.byMachine['1号机'], '分批导入不应漏掉 1号机订单');
assert.equal(chunkedM2, expectedStats.byMachine['2号机'], '分批导入不应漏掉 2号机订单');
