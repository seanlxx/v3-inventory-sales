...
function expectedImportStats(sourceRows) {
  const deviceToMachine = {
  ...
  for (const row of sourceRows) {
    const vendorOrderNo = String(row.vendorOrderNo || '').trim();
    if (vendorOrderNo) {
      currentOrder = {
        vendorOrderNo,
    ...
    const status = String(row.status || currentOrder?.status || '').trim();
    if (status && status !== '已完成') continue;
    const deviceCode = String(row.deviceCode || currentOrder?.deviceCode || '').trim();
    const machineId = deviceToMachine[deviceCode];
    if (!machineId) continue;
    const effectiveOrderNo = vendorOrderNo || currentOrder?.vendorOrderNo || '';
    if (!effectiveOrderNo || !row.vendorProductName) continue;
    if ((Number(row.quantity) || 0) <= 0) continue;

    if (!ordersByMachine.has(effectiveOrderNo)) ordersByMachine.set(effectiveOrderNo, machineId);
    lines += 1;
  }

  const byMachine = {};
  for (const machineId of ordersByMachine.values()) {
    byMachine[machineId] = (byMachine[machineId] || 0) + 1;
  }
  return {
    orders: ordersByMachine.size,
    lines,
    byMachine
  };
}
...
const envRefundedSale = { DB: new D1Database() };
loadBaseSchema(envRefundedSale);
const refundedSaleResult = await runZnImport(envRefundedSale, {
  orders: [
    {
      vendorOrderNo: 'refunded-sale-order',
      title: '',
      status: '已完成',
      deviceCode: 'TBN5CFA0261G547T5D3',
      vendorProductName: '怡宝饮用纯净水555ml',
      unitPrice: 2,
      quantity: 1,
      lineAmount: 6.5,
      receivedAmount: 6.46,
    ...
    },
    {
      ...
      status: '',
      deviceCode: '',
      vendorProductName: '东鹏补水啦电解质水柠檬味1L',
      unitPrice: 4.5,
      quantity: 1,
      lineAmount: 0,
      receivedAmount: 0,
...
assert.equal(refundedSaleResult.summary.ordersImported, 1, '已完成但含退款的原销售订单仍应导入');
assert.equal(refundedSaleResult.summary.linesImported, 2, '含退款原销售订单的一单多商品明细不应丢失');
const refundedSaleOrder = envRefundedSale.DB.queryOne(`
  SELECT total_amount_cents, received_amount_cents, refund_amount_cents
  FROM sales_orders
  WHERE id = ?
`, 'zn:refunded-sale-order');
assert.equal(refundedSaleOrder.total_amount_cents, 650, '含退款原销售单应保留成交金额');
assert.equal(refundedSaleOrder.received_amount_cents, 646, '含退款原销售单应保留平台成交/到账金额');
assert.equal(refundedSaleOrder.refund_amount_cents, 150, '含退款原销售单应先保留内嵌退款金额，退款明细导入时再去重');
...
await envRefunds.DB.prepare(`
  INSERT INTO sales_orders (
    ...
    500, 60, 0, 0, 0, 150, 500, '原销售', NULL, NULL, ?, ?,
...
assert.equal(
  envRefunds.DB.queryOne('SELECT refund_amount_cents FROM sales_orders WHERE id = ?', 'zn:visionpay-refund-original').refund_amount_cents,
  0,
  '退款明细导入后应清空原销售单内嵌退款金额，避免退款 KPI 重复统计'
);
...
const expectedStats = expectedImportStats(orders);
...
assert.equal(r1.summary.ordersImported, expectedStats.orders, '完整 Excel 应导入全部有效订单');
assert.equal(r1.summary.linesImported, expectedStats.lines, '完整 Excel 应按订单声明商品数导入有效商品明细');
...
const maySalesCount = env.DB.queryOne(
  'SELECT COUNT(*) AS n FROM sales_orders WHERE source = ? AND year_month = ?',
  'zn',
  '2026-05'
).n;
assert.equal(
  (await listSales(env, { yearMonth: '2026-05', status: 'active', limit: 500, offset: 0 })).length,
  Math.min(500, maySalesCount)
);
assert.equal(
  (await listSales(env, { yearMonth: '2026-05', status: 'active', limit: 500, offset: 500 })).length,
  Math.min(500, Math.max(0, maySalesCount - 500))
);

const multiItemOrderId = env.DB.queryOne(`
  ...
  LIMIT 1
`)?.id;
const multiItemOrder = multiItemOrderId ? env.DB.query(`
...
if (multiItemOrderId === 'zn:visionpaySFTS20260525083347733X') {
...
} else if (multiItemOrderId) {
  assert(multiItemOrder.length > 1, '真实 Excel 中的多商品订单应拆成多条明细');
  assert(
    multiItemOrder.every(item => item.line_amount_cents === item.unit_price_cents * item.quantity),
    '多商品订单每行金额应按单价 × 数量拆分'
  );
}
...
const settlementOrderBefore = env.DB.queryOne(`
  SELECT id, external_id, record_date, total_amount_cents, platform_fee_cents, service_fee_cents, received_amount_cents
  FROM sales_orders
  WHERE source = 'zn' AND total_amount_cents > 35
  ORDER BY id
  LIMIT 1
`);
const settlementGrossAmount = settlementOrderBefore.total_amount_cents / 100;
  ...
  settlements: [
    {
      ...
      expense: 0.35,
      incomeType: '收入',
      settledAt: `${settlementOrderBefore.record_date} 00:00:00`
    },
    {
...
assert.equal(
  settlementOrderAfter.received_amount_cents,
  settlementOrderBefore.total_amount_cents - 12 - 23,
  '交易账单实收应按 gross-refund-fee-service 计算'
);
assert.equal(
...
const m3 = env.DB.queryOne('SELECT COUNT(*) AS n FROM sales_orders WHERE machine_id = ?', '3号机').n;
console.log('1号机订单=%d  2号机订单=%d  3号机订单=%d', m1, m2, m3);
assert.equal(m1, expectedStats.byMachine['1号机'] || 0, '1号机有效订单应完整导入');
assert.equal(m2, expectedStats.byMachine['2号机'] || 0, '2号机有效订单应完整导入');
assert.equal(m3, expectedStats.byMachine['3号机'] || 0, '3号机有效订单应完整导入');
...
await env.DB.prepare('UPDATE products SET external_id = NULL, normalized_name = NULL WHERE id = ?')
  .bind(sampleProduct.id)
  .run();
...
const chunkedM3 = envChunked.DB.queryOne('SELECT COUNT(*) AS n FROM sales_orders WHERE machine_id = ?', '3号机').n;
assert.equal(chunkedSalesCount, expectedStats.orders, '分批导入也应导入全部有效订单');
assert.equal(chunkedM1, expectedStats.byMachine['1号机'] || 0, '分批导入不应漏掉 1号机订单');
assert.equal(chunkedM2, expectedStats.byMachine['2号机'] || 0, '分批导入不应漏掉 2号机订单');
assert.equal(chunkedM3, expectedStats.byMachine['3号机'] || 0, '分批导入不应漏掉 3号机订单');
...
const brokenOrderRows = brokenOrderStart >= 0
  ...
  : [
      {
        vendorOrderNo: 'visionpaySFTS20260525083347733X',
        title: '',
        status: '已完成',
        deviceCode: 'TBN5CFA0261G547T5D3',
        vendorProductName: '冰露饮用纯净水500ml',
        vendorBarcode: '',
        unitPrice: 1,
        quantity: 1,
        lineAmount: 6.5,
