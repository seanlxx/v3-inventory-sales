...
移动 token、`AppShell` 移动顶部栏 / 底部 Tab、`components/mobile/`（`MobileCard` / `MobileDataRow` / `MobileStateCard`）已落地，作为后续阶段的基底。
...
| 风险 | 处理方式 |
| --- | --- |
| `rpx` 与 `px` 视觉换算偏差 | 以 375px 为基准（`2rpx ≈ 1px`），再在 390/430px 微调 |
| 微信原生导航/TabBar 无法完全复刻 | CSS 固定顶部栏 + 底部 Tab，对齐文案、尺寸、颜色和安全区 |
| 浏览器与微信图片选择能力不同 | 用 `<input type="file" accept="image/*" capture="environment">`，预览/识别流程一致 |
| 桌面弹窗在手机上不适合 | 移动断点切换为全屏抽屉或独立移动视图 |
| 表格字段多导致溢出 | 移动端用卡片展示关键字段，详情页展示完整字段 |

## 9. 完成标准

- 移动浏览器首屏看起来像小程序，而不是桌面后台缩小版。
- 375 / 390 / 430px 全页面无横向溢出，底部安全区不遮挡。
- 顶部栏、底部 Tab、卡片列表、搜索、状态标签、按钮视觉接近小程序。
- 进货 / 销售 AI 识别流程可在手机浏览器完成。
- 所有数据仍来自现有 v3 后端，不新增 mock。
- 构建通过，并按 AGENTS.md §3.5 提交、推送和部署。
