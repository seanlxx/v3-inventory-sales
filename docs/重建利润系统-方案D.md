...
| 迁移数量差异 | `0` |
| `2026-07` 当前月成本缺口 | `0` |
| `2026-06` 历史成本缺口 | 已按用户确认接受为 `0` 成本占位 |
...
这些商品缺少真实成本凭证，已按用户确认写入 `manual_cost` 的 `0` 成本快照。系统不猜成本、不抬高利润，后期在进货页补录真实成本后再重新运行生产校验即可。当前它们不再作为成本缺口阻断项；旧库存表继续只归档保留，不物理删除。
...
- 未确认的成本缺失和迁移失败未清零前，不切换前端页面；已明确接受的历史 `0` 成本占位不计为未确认缺口。
