...
function summaryEntries(summary: VendorSyncSummary | null) {
  ...
  return [
    ...
    ['库存快照', summary.inventoryAdjusted],
    ['警告', summary.warnings]
  ] as const
}
...
<template>
  <SettingsSection title="轨道机手动同步" description="从盛码读取轨道机销售、商品售价与成本。">
    <template #aside>
    ...
    <div class="vendor-sync">
      ...
      <ul v-if="latestRun?.warnings?.length" class="vendor-sync__warnings">
        <li v-for="warning in latestRun.warnings.slice(0, 5)" :key="warning">
          {{ warning }}
        </li>
      </ul>
    </div>
