...
const maxRevenue = computed(() =>
  Math.max(...props.items.map(item => Number(item.revenue) || 0), 0)
)

function widthFor(item: MachineRankingItem) {
  if (!maxRevenue.value) return '0%'
  return `${Math.max(8, Math.round((item.revenue / maxRevenue.value) * 100))}%`
}
</script>

<template>
  <section class="machine-ranking surface-panel" aria-label="售货机销售排行">
    <header class="machine-ranking__header">
      <div>
        <h2 class="machine-ranking__title">售货机排行</h2>
        <p class="machine-ranking__description">按本月销售额排序</p>
      </div>
    </header>
    ...
    <div v-else class="machine-ranking__list">
      <article v-for="item in props.items" :key="item.machineId" class="machine-ranking__item">
        <div class="machine-ranking__row">
          ...
          <span class="numeric">{{ formatMoney(item.revenue) }}</span>
        </div>
